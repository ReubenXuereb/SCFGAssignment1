...
//When the Add AI button is pressed(this button can be found in GameManager Script) an AI is instantiated, then when the Start Button is pressed(this button can be found in GameManager Script)
//this script is then activated. The grid is always being scanned so when an obsatcel is instantiated in an AI's path the AI immidiatly changes its route towards the Target.
//The AI is moving one block at a time since in the coroutine it is always calculating each block that the green line(the path to the target) is passing through. Therefore it moves to one block at a time




public class AIMoveToWaypoint : MonoBehaviour
{
   ...
   //[SerializeField] Transform target;
    ...
    void Start()
    {
        ...
        StartCoroutine(moveTowardsTargets(this.transform));
    ...
    IEnumerator moveTowardsTargets(Transform t)
    {
    ...
    /*IEnumerator moveToNextWaypoint()
    {
        ...
        // pathToFollow = seeker.StartPath(transform.position, Targets[targetCounter++].position);
        StartCoroutine(moveTowardsTargets(this.transform));
        yield return null;
    }*/
